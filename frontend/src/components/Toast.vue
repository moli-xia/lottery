<template>
  <Teleport to="body">
    <TransitionGroup
      ...
      style="right: calc(1rem + env(safe-area-inset-right)); top: calc(1rem + env(safe-area-inset-top));"
    >
      <div
        v-for="toast in toasts"
        :key="toast.id"
        :class="[
          'min-w-[280px] max-w-md rounded-xl shadow-2xl backdrop-blur-lg border p-4 flex items-start gap-3',
          ...
          'bg-slate-700/90 border-slate-600/50 text-white'
        ]"
      >
...
const add = (message, type = 'info', description = '', duration = 4000) => {
  const id = nextId++
...
const success = (message, description = '') => add(message, 'success', description)
const error = (message, description = '') => add(message, 'error', description)
const warning = (message, description = '') => add(message, 'warning', description)
const info = (message, description = '') => add(message, 'info', description)
